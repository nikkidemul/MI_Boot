...
# Function for model development using Firth's logistic regression in every individual imputation set i using backward selection
...
refitmodelF <- function(impsets, formula){
...
#nullmodels to calculate IPA and rescaled Brier scores (using Firth) 
...
# For the calibration plot (Firth specific part): 
# provide coordinates of the smooth calibration curve ----
# This function is different from GLM because for GLM, 1 should be subtracted from the outcome because R transformed our outcome variable to 1 and 2 instead of 0 and 1
#### For Firth, this is not necessary. 
draw_smooth_calibrationF <- function( obs_outcome, pred_outcome){
  ...
  y_coordinates <- loess( y_input ~ x_input)$fitted 
