---
title: "3.4.3. LF_MIB_stability Firth"
author: "N. de Mul"
date: "2023-11-13"
...
predictedvalues <- predvalue(imputatiesets=Final8_impF, bsmodels = modelsbsimpF)
...
```{r}
Myimp1$lp <- -1.64555592 + 0.05114650*Myimp1$Leeftijd + 0.36984852*(as.numeric(Myimp1$T34)) + -0.41631040*(as.numeric(Myimp1$comorb_dm)) + 0.32709477*(as.numeric(Myimp1$dummy_Hblow)) + -1.00113037*(as.numeric(Myimp1$dummy_Neotx1)) + -0.01326058*Myimp1$fev1_compl + 0.30679485*(as.numeric(Myimp1$gender)) + 0.63520530*(as.numeric(Myimp1$open)) + -0.68291972*(as.numeric(Myimp1$transhiataal))

Myimp1$predF <- 1/(1+exp(-Myimp1$lp))
summary(Myimp1$predF)
```
...
Imp1 <- Myimp1 %>% filter(.imp=="1") %>% select(random_OKID, outcome, predF)
...
merged_df1 <- merged_df1 %>% select(random_OKID, outcome, predF, predbs)
```
...
model <- lm(predbs ~ predF, data=merged_df1)

#PERCENTIELEN berekenen: 
Q25 = aggregate(merged_df1$predbs, list(merged_df1$predF), quantile, 0.25)
Q75 = aggregate(merged_df1$predbs, list(merged_df1$predF), quantile, 0.75)

{plot(x=merged_df1$predF, 
                 y=merged_df1$predbs, 
...
MAPE: 
```{r}
merged_df1$MAPE <- merged_df1$predbs - merged_df1$predF
