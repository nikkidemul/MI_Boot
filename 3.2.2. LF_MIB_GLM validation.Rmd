...
saveRDS(mean_performanceglm,file=paste0("meanperformanceglm",".rds"))
```
...
var(calibrationimpintglm$imp) #variance is 0.007103361 --> 0.00795414 in simulation data ### Use this to calculate the confidence intervals, we chose to use percentiles (see below)
```
...
```{r}
#Intercept
quantile( calibrationimpintglm$imp, probs = 0.975) #0.2268 ### simulation data: 0.3446226 
quantile( calibrationimpintglm$imp, probs = 0.025) #-0.1056 ### simulation data: -0.03284604

#Slope
quantile(calibrationimpSglm$imp, probs=0.975) #1.029 ### simulation data: 0.9185784
quantile(calibrationimpSglm$imp, probs=0.025) #0.665 ### simulation data: 0.2765723
...
IPAupperGLM <- -0.03931757 + 1.96*sqrt(SEIPA)
IPAlowerGLM <- -0.03931757 - 1.96*sqrt(SEIPA)
...
-0.03931757 - 0.052370279

-0.09168785 + 1.96*sqrt(SEIPA)
-0.09168785- 1.96*sqrt(SEIPA)
...
OEupperGLM <- 2.12542358 + 1.96*sqrt(SEOE)
OElowerGLM <- 2.12542358 - 1.96*sqrt(SEOE)

#corrected for optimism (same method as described for IPA): 
2.12542358 - -0.001282099

2.126706 + 1.96*sqrt(SEOE)
2.126706 - 1.96*sqrt(SEOE)
```
...
BrierupperGLM <- 1.21951010 + 1.96*sqrt(SEBrier)
BrierlowerGLM <- 1.21951010 - 1.96*sqrt(SEBrier)
```
