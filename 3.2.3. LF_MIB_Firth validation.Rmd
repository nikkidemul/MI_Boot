...
impPerfBS <- function(imputatiesets, bootstrapsets, bsmodels, bsnullmodels){
  ...
  for(i in 1:length(imputatiesets)){
    ...
    null <- val.prob(bootstrapdf$prednull, as.numeric(bootstrapdf$outcome)-1, pl=FALSE)
    ...
    bootstrap <- val.prob(bootstrapdf$pred, as.numeric(bootstrapdf$outcome)-1, pl=FALSE)
    ...
    add.data.IPAbslogit <- data.frame(performanceM="logitIPA", bootstrap = logit(as.numeric(add.data.IPAbs$bootstrap)))
      add.dataOE <- data.frame(performanceM = "OE", bootstrap = as.numeric((mean(as.numeric(bootstrapdf$outcome)-1))/(mean(bootstrapdf$pred))))
    ...
    null <- val.prob(imputatieset$prednull, as.numeric(imputatieset$outcome)-1, pl=FALSE)
    ...
    imp <- val.prob(imputatieset$pred, as.numeric(imputatieset$outcome)-1, pl=FALSE)
    ...
    add.data.IPAimplogit <- data.frame(performanceM="logitIPA", imp = logit(as.numeric(add.data.IPAimp$imp)))
     add.dataOEimp <- data.frame(performanceM = "OE", imp = as.numeric(mean(as.numeric(imputatieset$outcome-1)))/(mean(imputatieset$pred)))
...
testperformanceF <- impPerfBS(imputatiesets=Final_impF, bootstrapsets=bootstrapsimp, bsmodels=modelsbsimpF, bsnullmodels=nullmodelsF)
...
### OLD SCRIPT FOR REFERENCE IF IT DOES NOT WORK
# mean_performanceF$plogismeanbs <- plogis(mean_performanceF$bootstrap)
# mean_performanceF$plogismeanimp <- plogis(mean_performanceF$imp)

saveRDS(mean_performanceF,file=paste0("meanperformanceF",".rds"))
```
...
var(calibrationimpintF$imp) #variance is 0.007103361 ### Use this to calculate the confidence intervals, we chose to use percentiles (see below)
```
...
```{r}
#Intercept
quantile( calibrationimpintF$imp, probs = 0.975) #0.2268
quantile( calibrationimpintF$imp, probs = 0.025) #-0.1056

#Slope
quantile(calibrationimpSF$imp, probs=0.975) #1.029
quantile(calibrationimpSF$imp, probs=0.025) #0.665
...
IPAupperF <- 0.01502440 + 1.96*sqrt(SEIPA)
IPAlowerF <- 0.01502440 - 1.96*sqrt(SEIPA)
...
0.01502440 - 0.009980300

0.0050441 + 1.96*sqrt(SEIPA)
0.0050441- 1.96*sqrt(SEIPA)
...
OEupperF <- 0.99022840 + 1.96*sqrt(SEOE)
OElowerF <- 0.99022840 - 1.96*sqrt(SEOE)

#corrected for optimism (same method as described for IPA): 
0.99022840 - -0.001274928

0.9915033 + 1.96*sqrt(SEOE)
0.9915033 - 1.96*sqrt(SEOE)
```
...
SEBrier # 5.338393e-05

BrierupperF <- 1.21951010 + 1.96*sqrt(SEBrier)
BrierlowerF <- 1.21951010 - 1.96*sqrt(SEBrier)
```
