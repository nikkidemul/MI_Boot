...
#Function for GLM model development using backward selection
...
# Calculation of predicted values in each imputed set, to later be able to calculate performance measures: 
...
# This function is different from Firth due to the -1 on the outcome (because R transformed our outcome variable to 1 and 2 instead of 0 and 1)
draw_smooth_calibration <- function( obs_outcome, pred_outcome){
  ...
  y_coordinates <- loess( (as.numeric(y_input)-1) ~ x_input)$fitted 
