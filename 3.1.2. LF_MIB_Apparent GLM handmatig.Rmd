...
Save file: 
```{r}
saveRDS(finalmodelimpglm3, file=paste0("FinalmodelGLMsim"))
```
...
impPerf <- function(impsetspred){
  ...
  for(i in 1:length(impsetspred)){
    ...
    performanceimp <- val.prob(totalpred$pred, as.numeric(totalpred$outcome)-1, pl=FALSE)
    ...
    performancenull <- val.prob(totalpred$prednull, as.numeric(totalpred$outcome)-1, pl=FALSE)
    ...
    add.data.OE <- data.frame(performanceM = "OE", performanceimp =  (mean((as.numeric(totalpred$outcome)-1)))/(as.numeric(mean(as.numeric(totalpred$pred)))))
...
#simulation data
plogis(logit(0.63) - 1.96*sqrt(0.0105969))
plogis(logit(0.63) + 1.96*sqrt(0.0105969))
```
Save performance output: 
```{r}
saveRDS(finalperformanceGLM, file=paste0("finalappperformanceGLMsim"))
```
...
Myimp$lpGLM <- 2.22187840	 + 0.03613563*Myimp$age + 0.32613135*as.numeric(Myimp$comorb_hypertension) + -3.07940731*as.numeric(Myimp$eGFR) + -0.01563689*Myimp$fev1_compl + 0.57910986*as.numeric(Myimp$gender) + -0.88026153*as.numeric(Myimp$transhiatal) + 0.49382807*as.numeric(Myimp$smoking)
